...
export class MCPConfigManager extends EventEmitter {
  ...
  constructor(configPath: string = '.mcp-config.json', autoSave: boolean = true) {
    super();
    ...
    const builtInPresets: MCPConfigPreset[] = [
      {
        ...
        description: 'Configuration for blockchain and smart contract development',
        author: 'Solmix IDE',
        version: '1.0.0',
      ...
      },
      {
        ...
        description: 'Complete development environment with filesystem, git, and web tools',
        author: 'Solmix IDE',
        version: '1.0.0',
      ...
      },
      {
        ...
        description: 'Development environment with AI tools and web search capabilities',
        author: 'Solmix IDE',
        version: '1.0.0',
  ...
  public createPreset(preset: Omit<MCPConfigPreset, 'id' | 'createdAt' | 'updatedAt'>): string {
  ...
  public updatePreset(presetId: string, updates: Partial<Omit<MCPConfigPreset, 'id' | 'createdAt'>>): boolean {
  ...
  public validatePreset(preset: MCPConfigPreset): MCPConfigValidationResult {
    ...
    if (!preset.name.trim()) {
      errors.push({ field: 'name', message: 'Preset name is required', severity: 'error' });
    }
  ...
  public validateGlobalConfig(config: MCPGlobalConfig): MCPConfigValidationResult {
    const errors: Array<{ field: string; message: string; severity: 'error' | 'warning' }> = [];
    const warnings: Array<{ field: string; message: string }> = [];

    if (config.defaultTimeout < 1000) {
      warnings.push({ field: 'defaultTimeout', message: 'Very short timeout may cause connection issues' });
    }
    
    if (config.maxConcurrentConnections > 20) {
      warnings.push({ field: 'maxConcurrentConnections', message: 'High connection limit may impact performance' });
    }
    
    if (config.retryAttempts > 10) {
      warnings.push({ field: 'retryAttempts', message: 'High retry count may cause delays' });
    }
  ...
  public restoreBackup(backupId: string): boolean {
    ...
    // Verify checksum
    const currentChecksum = this.calculateChecksum(JSON.stringify(backup.config));
    if (currentChecksum !== backup.checksum) {
      throw new Error('Backup integrity check failed');
    }
    
    // Create current state backup before restore
    this.createBackup(`Auto-backup before restore from ${backup.name}`, 'Automatic backup created before restore operation');
  ...
  public importConfig(configJson: string): void {
    try {
      ...
      if (!importData.version || !importData.global || !Array.isArray(importData.presets)) {
        throw new Error('Invalid configuration format');
      }
      
      // Create backup before import
      this.createBackup('Auto-backup before import', 'Automatic backup created before configuration import');
      
      // Import global config
      this.globalConfig = { ...this.getDefaultGlobalConfig(), ...importData.global };
      ...
      if (importData.presets) {
        importData.presets.forEach((preset: MCPConfigPreset) => {
          ...
          if (validation.valid) {
            ...
            console.warn(`Skipping invalid preset ${preset.name}:`, validation.errors);
          }
        });
      }
      
      // Set active preset if valid
      if (importData.activePreset && this.presets.has(importData.activePreset)) {
...
export function getMCPConfigManager(configPath?: string, autoSave?: boolean): MCPConfigManager {
