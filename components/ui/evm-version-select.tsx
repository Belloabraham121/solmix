...
export function EVMVersionSelect({ value, onValueChange, disabled }: EVMVersionSelectProps) {
    ...
    <Select value={value} onValueChange={handleChange} disabled={disabled}>
      ...
      <SelectContent className="bg-slate-800 border-slate-700 min-w-[250px]">
        {evmVersions.map((version) => (
          <SelectItem key={version.value} value={version.value} className="text-sm py-3 px-3 hover:bg-slate-600 focus:bg-slate-600">
            <div className="flex flex-col gap-1">
              <span className="font-medium text-white">{version.name}</span>
              <span className="text-slate-200 text-sm">{version.description}</span>
            </div>
          </SelectItem>
        ))}
      </SelectContent>
